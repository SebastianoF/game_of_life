...
class StateManager(object):
    ...
    def loader(self, time_state):
        ...
        :return:
        """
        filename = join(self.path_to_game_folder, self.game_name + '_t' + str(time_state) + '.txt')
        arr = np.loadtxt(filename)
        arr.astype(np.uint8)
        return arr  # the np.ndarray

    def saver(self, input_array_state, time_state=0, check=True):
        """
        ...
        print('Input state generated.')

    def initialise_random(self, n1, n2, ones_percentage=1./10):
        """
